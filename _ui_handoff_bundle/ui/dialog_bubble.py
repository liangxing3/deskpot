...
class DialogBubble(QWidget):
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self.label.setStyleSheet("color: #5A5A5A; line-height: 140%;")
        font = QFont("Segoe UI Emoji", 10)
        font.insertSubstitutions("Segoe UI Emoji", ["Microsoft YaHei UI"])
    ...
    def paintEvent(self, event) -> None:  # noqa: N802
        ...
        path.moveTo(rect.center().x() - 8, rect.bottom())
        path.lineTo(rect.center().x(), rect.bottom() + 8)
        path.lineTo(rect.center().x() + 8, rect.bottom())
        path.closeSubpath()

        painter.fillPath(path, QColor(255, 253, 254, 245))
        painter.setPen(QPen(QColor(255, 207, 223, 200), 1.5))
