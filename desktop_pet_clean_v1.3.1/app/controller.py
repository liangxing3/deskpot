...
class AppController(QObject):
    ...
    def _connect_signals(self) -> None:
        ...
        self.window.drag_started.connect(self._handle_pet_drag_started)
        self.window.drag_finished.connect(self._handle_pet_drag_finished)
    ...
    def _handle_pet_clicked(self) -> None:
        if self.animation_manager.is_overlay_active():
            return
        self._note_activity()
    ...
    def _handle_pet_drag_started(self) -> None:
        should_schedule_startup_greeting = (
            self._startup_greeting_waiting_for_appear
            and not self._startup_greeting_shown
            and self.animation_manager.current_animation == PetAnimationKey.APPEAR
        )
        transition = self.animation_manager.start_overlay_animation(PetAnimationKey.DRAG)
        if transition is not None:
            self._apply_transition(transition)
        if should_schedule_startup_greeting:
            self._schedule_startup_greeting(delay_ms=STARTUP_GREETING_DELAY_MS)

    @Slot(object)
    def _handle_pet_drag_finished(self, _position: WindowPosition | None = None) -> None:
        transition = self.animation_manager.stop_overlay_animation()
        if transition is not None:
            self._apply_transition(transition)
            return
        self._refresh_base_animation(current_time=now_local())
