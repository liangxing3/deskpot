...
class WeatherDialog(DialogShell):
    ...
    def _build_hero_card(self) -> QWidget:
        ...
        card.setStyleSheet(
            f"QWidget {{{soft_card_stylesheet()} background: {_rgba(Colors.BG_INPUT, 232)}; border: 1px solid {_rgba(Colors.BORDER_DEFAULT, 88)};}}"
        )
    ...
    def _build_status_card(self) -> QWidget:
        card = QWidget(self.content_panel)
        card.setStyleSheet(
            f"QWidget {{{soft_card_stylesheet()} background: {_rgba(Colors.BG_INPUT, 236)}; border: 1px solid {_rgba(Colors.BORDER_DEFAULT, 92)};}}"
        )
    ...
    def _metric_card(self, title: str) -> tuple[QLabel, QWidget]:
        card = QWidget(self.content_panel)
        card.setStyleSheet(
            f"QWidget {{{soft_card_stylesheet()} background: {_rgba(Colors.BG_CARD, 252)}; border: 1px solid {_rgba(Colors.BORDER_DEFAULT, 76)};}}"
        )
        layout = QVBoxLayout(card)
    ...
    def _show_status_card(self, title: str, message: str, *, error: bool) -> None:
        ...
        self.status_card.setStyleSheet(
            f"QWidget {{{soft_card_stylesheet()} background: {_rgba(Colors.BG_INPUT, 236)}; border: 1px solid {_rgba(Colors.BORDER_DEFAULT, 92)}; border-left: 3px solid {accent.name()};}}"
        )
...
def _content_panel_stylesheet() -> str:
    return (
        ...
        f"border: 1px solid {_rgba(Colors.BORDER_DEFAULT, 92)};"
