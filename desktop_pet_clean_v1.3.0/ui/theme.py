...
def dialog_shell_stylesheet() -> str:
    ...
    QWidget#DialogSurface {{
        ...
        border: 1px solid {_rgba(c.BLUSH, 210)};
    ...
    QWidget#DialogHeader {{
        ...
        border: none;
        border-bottom: 1px solid {_rgba(c.BLUSH, 140)};
        outline: none;
    }}
    QLabel#DialogTitle {{
        color: {c.TEXT_PRIMARY.name()};
    ...
    QPushButton#DialogClose {{
        ...
        border: 1px solid {_rgba(c.BLUSH, 190)};
        border-radius: 10px;
...
def soft_card_stylesheet() -> str:
    ...
    border: 1px solid {_rgba(c.BORDER_DEFAULT, 95)};
    border-radius: {m.RADIUS_MD}px;
    outline: none;
    """
...
def soft_button_stylesheet(*, primary: bool = True) -> str:
    ...
    QPushButton:disabled {{
        background: {_rgba(c.BORDER_DEFAULT, 60)};
        color: {_rgba(c.TEXT_SECONDARY, 180)};
        border-color: {_rgba(c.BORDER_DEFAULT, 70)};
    }}
    """
...
def line_edit_stylesheet() -> str:
    ...
    QLineEdit, QComboBox, QSpinBox, QTextEdit {{
        ...
        border: 1px solid {_rgba(c.BLUSH, 200)};
        border-radius: {Metrics.RADIUS_SM}px;
        padding: 8px 10px;
        color: {c.TEXT_PRIMARY.name()};
        font-size: {Typography.SIZE_BODY}px;
    ...
    QLineEdit:focus, QComboBox:focus, QSpinBox:focus, QTextEdit:focus {{
        border-color: {c.ROSE.name()};
        background: {_rgba(c.CREAM, 255)};
    }}
...
def segmented_button_stylesheet() -> str:
    ...
    QPushButton:checked {{
        background: {_rgba(c.BLUSH_SOFT, 245)};
        border-color: {_rgba(c.BLUSH, 220)};
        color: {c.ROSE_DARK.name()};
        font-weight: 600;
    }}
...
def toggle_button_stylesheet() -> str:
    ...
    return f"""
    QPushButton {{
        border: 1px solid {_rgba(c.BORDER_DEFAULT, 120)};
        border-radius: 12px;
        min-width: 58px;
        max-width: 58px;
        min-height: 26px;
